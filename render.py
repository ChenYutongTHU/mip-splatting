...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, kernel_size, scale_factor, save_as_idx=False):
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, save_as_idx : bool):
    with torch.no_grad():
        ...
        if not skip_train:
            ...
            ), gaussians, pipeline, background, kernel_size, scale_factor=scale_factor)
            ...
            if type(scene.test_cameras) == dict:
                for test_name in scene.test_cameras.keys():
                    render_set(dataset.model_path, f"{test_name}-{dataset.focal_length_scale}_minus-depth-{dataset.minus_depth:.2f}", scene.loaded_iter, scene.getTestCameras(
                        test_name=test_name), gaussians, pipeline, background, kernel_size, scale_factor=scale_factor)
            else:
