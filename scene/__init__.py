...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        for resolution_scale in resolution_scales: #[1.0]
            ...
            self.train_cameras[resolution_scale] = cameraList_from_camInfos(scene_info.train_cameras, resolution_scale, args, is_training=shuffle, point_cloud=scene_info.point_cloud.points)
            print(f"#={len(self.train_cameras[resolution_scale])}")
            if isinstance(scene_info.test_cameras, list):
                self.test_cameras['test'][resolution_scale] = cameraList_from_camInfos(scene_info.test_cameras, resolution_scale, args, is_training=False, point_cloud=scene_info.point_cloud.points)
