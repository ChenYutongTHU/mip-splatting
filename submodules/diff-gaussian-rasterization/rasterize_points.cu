	...
	const bool debug)
{
  ...
  torch::Tensor out_depth_mode = torch::full({1, H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		out_depth.contiguous().data<float>(), out_depth_mode.contiguous().data<float>(),
		out_alpha.contiguous().data<float>(),
		radii.contiguous().data<int>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, out_depth, out_depth_mode, out_alpha, radii, geomBuffer, binningBuffer, imgBuffer);
}
